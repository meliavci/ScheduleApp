...
### Project Setup
This section details the project management infrastructure we established using Azure DevOps (ADO). We transitioned from informal task tracking to a structured Application Lifecycle Management (ALM) process to ensure traceability and collaboration.
...
1.  **Project Creation:** Melisa created a new project within our organization’s Azure DevOps instance named "ScheduleApp".
2.  **Member Assignment:** Melisa then added all group members to the ADO project team.
3.  **Permission Management:** We encountered a specific hurdle where the default "Basic" role did not grant members access to the "Repos" section. As the project administrator, Melisa had to manually elevate permissions for the rest of the team to ensure everyone could view the Git history and contribute to the code, which still didn't work. So it resulted in Melisa being the only one who had access to the "Repos" section in our Azure project and needed to manage everything.

#### GitHub Integration and Solution Binding
The core of our version control strategy was linking our local Power Apps development environment to the remote Git repository managed through Azure. This allows us to track granular changes to the low-code files.
...
*   **Version Control Mechanism**
    ...
    - **Committing Changes:** When a developer was ready to share their work, the changes were saved in the Power Apps solution and then manually committed to the Azure Repo.
    ...
    - **Centralized Management:** Since Melisa was the only one who could initially manage the repository, commits were reviewed by the group before being merged into the main development branch.
...
As shown in the figures below, the project was successfully set up and linked.

![Azure DevOps Project Overview](../assets/Assignment3/AzureBoard.png)

*Azure DevOps project overview showing project statistics.*

![Github Repo](../assets/Assignment3/GithubRepo.png)

*The linked repository showing the project files.*


### Work Items Definition
To ensure traceability, we defined our work items using a hierarchical structure: **Epics → Issues → Tasks**.

**Table: Epic Definition**
...
We utilized the Kanban board view in Azure DevOps to visualize the workflow. The screenshots below show the different states of our tasks.

![Azure Boards Kanban 1](../assets/Assignment3/AzureBoardIssues.png)

*Azure Boards Kanban view - Issue tracking.*

![Azure Boards Kanban 2](../assets/Assignment3/AzureBoardTasks1.png)

*Azure Boards Kanban view - Tasks*

![Azure Boards Kanban 3](../assets/Assignment3/AzureBoardTasks2.png)

*Azure Boards Kanban view - Tasks*

### Planning and Collaboration
We utilized **Azure Boards** to plan, manage, and visualize our teamwork.
...
We mapped our project requirements from the highest level down to execution:
*   **Epics:** Represented the major overarching goals (e.g., "Schedule Optimization App").
*   **Issues:** Defined core functional requirements from the user's perspective (e.g., "The professor wants to cancel a class").
*   **Tasks:** Detailed the concrete technical steps required to complete an Issue (e.g., "Create Dataverse table: Person", "Implement Back-end Logic for Cancellation").
...
This section describes the technical implementation of our deployment pipeline, which automates the movement of our application between environments.
...
**Table: Pipeline Environment Strategy**
...
We utilized the native **Power Platform Pipelines** to orchestrate deployments. As shown in **Power Apps pipeline configuration**, the pipeline moves the solution sequentially from Development to Testing, and finally to Production.

![Pipeline Setup](../assets/Assignment3/PowerAppsPipelines.png)

*Power Apps pipeline configuration showing the flow from Development to Production.*

#### What does each stage do?
*   **Development Stage:** This is where the application and all its components are initially built, configured, and refined. Developers work with unmanaged solutions, allowing full flexibility for design changes, testing, and iteration.
*   **Testing Stage:** At this point, the solution is deployed as a managed version for evaluation and user testing. The goal is to verify that all features function correctly and that the app behaves as expected before release.
*   **Production Stage:** This final stage hosts the approved solution used by end users. Since it contains managed solutions, direct editing is disabled to protect stability and ensure a consistent production environment.

#### How is deployment triggered?
Deployments are **manually initiated** from the Development environment. A developer selects the solution and clicks "Deploy" to move it to the Testing stage. This manual gate serves as a quality check, ensuring code is not pushed while it is still broken or incomplete.
*   **Version Control Enforcement:** The pipeline enforces versioning by requiring an incremented solution version number (e.g., `1.0.0.1` → `1.0.0.2`) for every deployment. This creates a traceable history of changes, allowing us to identify exactly which version is running in Production.
...
*   **Limittations and Challenges**
    ...
    *   **Visibility Restrictions:** Initially, not all group members could see the created pipelines in the Maker Portal due to permission settings in the host environment. We had to adjust security roles to enable collaboration.
...
### Flow Description
We implemented two specific cloud flows to handle logic that the Power Apps frontend cannot perform securely or automatically. These flows are directly linked to the process model created in Assignment 1.

#### Flow Analysis: Cancel_Course_Notification

**Flow Description:**
This instant cloud flow is triggered manually from the Power App (by the professor) to instantly cancel a selected class session in the backend and notify all affected students. It corresponds directly to the **"Cancel Class"** task in the **Professor** swimlane of our BPMN diagram.

**Short Explanation:**
*   **What does the flow automate?** The flow automates one step: Updating the course status (Column: “Taking place?”) in the Dataverse Courses table to "No" (Cancelled).
...
The logic of the flow is illustrated below in **Logic for the 'Cancel_Course_Notification' flow.**.

![Cancel Course Flow](../assets/Assignment3/CancelCourseFlow.png)

*Logic for the 'Cancel_Course_Notification' flow.*

#### Flow Analysis: Reset_Cancelled_Courses_Daily

**Flow Description:**
This scheduled cloud flow runs once daily to clean up the schedule by resetting the status of courses whose cancellation day has passed, ensuring they are automatically available for the next scheduled week. This flow corresponds to the **System** swimlane activity of maintaining data integrity.
...
The logic of this scheduled flow is shown in **Trigger and List Rows logic for the 'Reset_Cancelled_Courses_Daily' flow.** and **Condition and Update logic for the 'Reset_Cancelled_Courses_Daily' flow.**.

![Reset Course Flow Part 1](../assets/Assignment3/ResetCancelledCourseFlow.png)

*Trigger and List Rows logic for the 'Reset_Cancelled_Courses_Daily' flow.*

![Reset Course Flow Part 2](../assets/Assignment3/ResetCancelledCourseFlow2.png)

*Condition and Update logic for the 'Reset_Cancelled_Courses_Daily' flow.*
...
1.  **Reliable Core Functionality:**
    ...
    *   **Managed Solution Integrity:** We successfully configured the pipeline to deploy the solution artifact sequentially across environments as a Managed Solution. This guarantees that the exact version validated in Testing is moved to Production, which is crucial for maintaining data integrity and system stability.
    *   **Version Control Enforcement:** The requirement to increment the solution version number before deployment enforced a critical best practice. This ensures every release is traceable and helps us manage clear tracking and identification of changes.
...
2.  **Efficient Task Automation:**
    ...
    *   **Instant Communication (Cancellation Flow):** We implemented a flow triggered directly from the Power App that instantly updates the course status to 'Cancelled'. This fulfilled the need for immediate, automatic communication and solved the problem of communication delays.
    *   **Automated Data Hygiene (Reset Flow):** We created a scheduled flow that runs daily at a specific time (e.g., 23:45) to automatically reset the status of cancelled courses. This solved the problem of manual data maintenance, ensuring that course availability is correctly restored for the following week.
...
3.  **Component Binding:** Establishing the dynamic visibility of the notification icon required creating a complex Custom Property (`IsCancelledAlert`) and resolving various circular reference and syntax errors to correctly pass the calculated count to the component.
